...
export function ServicesPanel({ query }: Props) {
  const [items, setItems] = useState(repo.services.list());
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) =>
      [item.name, item.category, item.description ?? ""].some((field) => field.toLowerCase().includes(q)),
    );
  }, [items, query]);
  ...
  const onSave = (formData: FormData) => {
    ...
    setErrors({});
    repo.services.upsert(parsed.data as Service);
    setItems(repo.services.list());
  ...
  return (
    <section className="card">
          ...
          <tbody>
            {paged.map((s) => (
              <tr key={s.id} className="border-t">
                ...
                <td className="max-w-[220px] truncate p-3 text-gray-700">{s.description}</td>
      ...
      <Modal
        open={modalOpen}
        title={editing ? "Засах" : "Шинээр нэмэх"}
      ...
      >
        <form className="grid gap-3" action={onSave} id={formId}>
          ...
          <label className="grid gap-1 text-sm">
            ...
            {errors.durationMin && <p className="text-xs text-red-600">{errors.durationMin}</p>}
          </label>
          <label className="grid gap-1 text-sm">
            Төрөл
            ...
            {errors.kind && <p className="text-xs text-red-600">{errors.kind}</p>}
          </label>
          <label className="grid gap-1 text-sm">
            Тайлбар
